context("Login", () => {
  ...
  it("Log out", () => {
    cy.get('[data-testid="header"] [data-testid="profile-image"]')
      .eq(0)
      .trigger("mouseover");
    cy.contains("Find your perfect design").should("be.visible");
    cy.get('[data-testid="user-info"]').should("be.visible");
    cy.get('[data-testid="log-out"]').click();
    cy.contains("Log In").should("be.visible");
    cy.contains("Sign Up").should("be.visible");
    cy.contains("Find your perfect design").should("not.exist");
  });
});
